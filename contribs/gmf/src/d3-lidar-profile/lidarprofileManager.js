...
gmf.lidarProfile.Manager = class {
  ...
  */   
  ...
  init(options) {
    ...
    this.utils_ = new gmf.lidarProfile.utils(options, null); //FIXME
    this.loader = new gmf.lidarProfile.loader(options, this.plot); //FIXME
    this.measure = new gmf.lidarProfile.measure(options, this); //FIXME
  };
  ...
  pointHighlight() {
    ...
    let cx, cy;
    const p = this.utils_.getClosestPoint(this.loader.profilePoints,
    ...
    if (p != undefined) {
      ...
      d3.select('#profileInfo')
        .html(html);
      this.loader.cartoHighlight.setElement(null);
      const el = document.createElement('div');
      el.className += 'tooltip gmf-tooltip-measure';
      el.innerHTML = html;
  
      this.loader.cartoHighlight.setElement(el);
      this.loader.cartoHighlight.setPosition([p.coords[0], p.coords[1]]);
      const classifColor = this.options.profileConfig.classification[p.classification].color;
      this.loader.lidarPointHighlight.getSource().clear();
      ...
      this.loader.lidarPointHighlight.getSource().addFeature(lidarPointFeature);
    } else {
      this.loader.lidarPointHighlight.getSource().clear();
      svg.select('#highlightCircle').remove();
      d3.select('#profileInfo').html('');
      this.loader.cartoHighlight.setPosition(undefined);
    }
  };
  ...
  */
  changeStyle(material) {
    ...
    this.drawPoints(this.loader.profilePoints, material);
  };
